...
describe('Parsing Typescript file', () => {
  it('should identify all exported functions defined in the file', async () => {
    ...
    const expectedFunctionNames = [
      ['test.testFun1', ['constFunction', 'objTest2.test']],
      ['test.testFun2', ['constFunction', 'anotherType', 'constFunction2', 'traditionalFunction2']],
      ['test.constFunction', []],
      ['test.constFunction2', []],
      ['test.anotherType', []],
      ['test.anotherType2', []],
    ...
    result.forEach((r, i) => {
      expect(r.function).toEqual(expectedFunctionNames[i][0]);
      expect(r.handler).toEqual(expectedFunctionNames[i][1]);
    });
  });
});
